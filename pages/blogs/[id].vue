      ...
      <div class="card-actions items-center border-t-2 pt-2">
        <div class="flex-1 items-center" v-if="post.user">
          <div class="flex items-center">
            <NuxtImg v-if="post.user && post.photos.length"
              class="w-12 rounded-full mr-4"
              :src="post.photos[0].url"
              :alt="post.user.username"
            />
            <div class="text-sm">
              <p class="text-gray-900 leading-none"><NuxtLink target="_blank" :to="'https://'+post.user.website">@{{ post.user.username }}</NuxtLink></p>
              <p class="text-gray-600"><NuxtLink :to="'https://'+post.user.website" target="_blank">{{ post.user.website }}</NuxtLink></p>
            </div>
          </div>
        </div>
        <div class="justify-end flex-wrap">
          <div class="badge badge-outline ms-1" v-for="splittedName in post.title?.split(' ')"><NuxtLink :to="'/blogs/search/'+splittedName">{{ splittedName }}</NuxtLink></div>
        </div>
      </div>
    </div>
  </div>
